...
df <- df %>% mutate_if(~n_distinct(.[]) > 50, as.numeric)
...
df <- data.frame(num_cols,fact_cols)
...
col_names_removed <- setdiff(original_col_names, names(df))
col_names_removed <- stri_join_list(list(col_names_removed), sep = ",  ")
col_names_removed <- paste("variables removed:  ", col_names_removed)
print(col_names_removed)

write.Alteryx(col_names_removed, 3)
